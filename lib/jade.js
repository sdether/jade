...
function compile(parseResult, options) {
  ...
  if (options.compileDebug !== false) {
    fn = [
      ...
      , parseResult.parsed
      , '} catch (err) {'
      , '  jade.rethrow(err, jade.debug[0].filename, jade.debug[0].lineno' + (options.compileDebug === true ? ',' + JSON.stringify(parseResult.source) : '') + ');'
      , '}'
    ].join('\n');
  } else {
...
exports.analyze = function(str, options) {
  options = options || {};
